...
class Temp:
    # currently just hardcoded the paragraph content
...
def editor(request):
    ...
    if request.method == 'POST':
        ...
        if title == "":
            title = "No Title" # do not want to save with empty title
...
def summarize_text(request):
    """Use BART model to summarize the quote"""
    if request.method == 'POST':
        import json
        data = json.loads(request.body)
        text = data.get('text', '')

        # Tokenize and summarize the quote
        inputs = tokenizer.encode("summarize: " + text, return_tensors='pt', max_length=1024, truncation=True)
        summary_ids = model.generate(inputs, max_length=150, min_length=40, length_penalty=2.0, num_beams=4, early_stopping=True)
        summary = tokenizer.decode(summary_ids[0], skip_special_tokens=True)
        return JsonResponse({'summary': summary})
    return JsonResponse({'error': 'Invalid request method'}, status=400) # should add more error handling




def update_paragraph(request):
    """Update the paragraph content to include highlights"""
    if request.method == 'POST':
